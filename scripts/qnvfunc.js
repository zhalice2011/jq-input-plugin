...
function TV_Initialize() {
	...
	try{
    	   ...
    	   var obj = document.getElementById("qnviccub");
    	   if (obj.QNV_DevInfo(0, QNV_DEVINFO_GETCHANNELS) <= 0) {
	    obj.QNV_OpenDevice(0, 0, 0);
		//初始化状态控制
	    var channels = obj.QNV_DevInfo(0, QNV_DEVINFO_GETCHANNELS);
		if(channels > 0)
 		{
 		    AppendStatus("打开设备成功 通道数:" + channels + " 序列号:" + obj.QNV_DevInfo(0, QNV_DEVINFO_GETSERIAL) + " 设备类型:" + obj.QNV_DevInfo(0, QNV_DEVINFO_GETTYPE) + " ver:" + obj.QNV_DevInfo(0, QNV_DEVINFO_FILEVERSION));	
 		}else
		{
...
function TV_Disable() {
    ...
    if(null != obj)
    {
        obj.QNV_CloseDevice(ODT_ALL, 0); //关闭所有设备
        AppendStatus("关闭设备完成.");
    }
}
...
function TV_EnableHook(uID,bEnable) {
    var obj = document.getElementById("qnviccub");
    obj.QNV_SetDevCtrl(uID, QNV_CTRL_DOHOOK, bEnable);
...
function TV_StartDial(uID,szCode) {//正常拨号必须使用 DIALTYPE_DTMF
    var obj = document.getElementById("qnviccub");
    if (obj.QNV_General(uID, QNV_GENERAL_STARTDIAL, DIALTYPE_DTMF, szCode) <= 0) {
