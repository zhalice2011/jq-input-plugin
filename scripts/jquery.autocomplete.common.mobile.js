...
$.Autocompleter = function(input, options) {
    ...
    $(input)[0].onblur=function() {
     ...
     $(input)[0].onclick=function() {
		// show select when clicking in a focused field
        var visible = select.visible() ;
		if ( hasFocus++ > 1 && visible ) {
            //alert('onch') ;
			onChange(0, true);
		}
	};
...
//alert('.Autocompleter.Select') ;
	...
	function init() {
		...
		element = $("<div/>");
		element.hide();
		element.addClass(options.resultsClass);
	...
	function fillList() {
		...
		//alert('data[i].value:'+data[i].value) ;
            ...
            var li = $("<li/>");
            li.html( options.highlight(formatted, term) ).addClass(i%2 == 0 ? "ac_even" : "ac_odd") ;
            list.append(li) ;
            //li=li[0] ;
            //alert('1') ;
	...
	return {
		...
		visible : function() {
            ...
            //return element;
            return element&&(element[0].style.visibility=="visible"||element[0].style.visibility=="") ;
		},
		current: function() {
		...
		show: function() {
            ...
            var test = $(input)[0].style.width ;
            //alert(test) ;
            var inputWidth = input.offsetWidth;;//$(input)[0].style.width;
            var width = options.width > 0 ? options.width : inputWidth;
		...
		selected: function() {
            ...
            var resultItem = $(selected[0]).data("ac_data") ;
			//return selected && selected.length && $.data(selected[0], "ac_data");
            return selected && selected.length && resultItem;
		},
		emptyList: function (){
			list && list.empty();
		},
