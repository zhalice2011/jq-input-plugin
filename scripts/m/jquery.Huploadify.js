            ...
            var uploadManager = {
                ...
                init: function () {
                    ...
                    if (fileInput.length > 0) {
                        ...
                        fileInput[0].onchange = function (e) { uploadManager._getFiles(e); };
                    ...
                    _this.find('.uploadify-button').on('click', function () {
                        ...
                        var a = _this.find('.selectbtn');
                        //_this.find('.selectbtn').click();
                        a[0].click();
                    });
                    ...
                    if (typeArray.length > 0) {
                        for (var i = 0, len = files.length; i < len; i++) {
                            ...
                            if (parseInt(F.formatFileSize(f.size, true)) <= option.fileSizeLimit) {
                                ...
                                //if (true)
                                if ($.inArray('*', typeArray) >= 0 || $.inArray(lowerFileName.split('.').pop(), typeArray) >= 0) //continue modify
                                {
                                    arr.push(f);
                                }
                                else {
                                    alert('文件 "' + f.name + '" 类型不被允许！');
                                }
                            }
                            else {
                ...
                _renderFile: function (file) {
                    ...
                    var parentElement = $html.find('.uploadify-progress');
                    //判断是否显示已上传文件大小
                    if (option.showUploadedSize) {
                        ...
                        //$html.find('.uploadify-progress').after(num);//continue modify
                        insertAfter($(num)[0], parentElement[0]);
                    }

                    //判断是否显示上传百分比	
                    if (option.showUploadedPercent) {
                        var percentText = '<span class="up_percent">0%</span>';
                        //$html.find('.uploadify-progress').after(percentText);//continue modify
                        insertAfter($(percentText)[0], parentElement[0]);
                    }
                ...
                onProgress: function (file, loaded, total) {
                    ...
                    if (option.showUploadedSize) {
                        eleProgress.parent().find('.progressnum .uploadedsize').text(F.formatFileSize(loaded));
                        eleProgress.parent().find('.progressnum .totalsize').text(F.formatFileSize(total));
                    }
                    if (option.showUploadedPercent) {
                        eleProgress.parent().find('.up_percent').text(percent);
                    }
                        ...
                        xhr.onreadystatechange = function (e) {
                            if (xhr.readyState == 4) {
                                if (xhr.status == 200) {
                                    ...
                                    setTimeout(function () {
                                        //_this.find('#fileupload_' + instanceNumber + '_' + file.index).fadeOut();
                                        _this.find('#fileupload_' + instanceNumber + '_' + file.index).hide();
