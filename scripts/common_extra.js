...
function _relatedlinks(rlinkmsgid) {
	if(!discuz$(rlinkmsgid) || discuz$(rlinkmsgid).innerHTML.match(/<script[^\>]*?>/i)) {
		return;
	}
	var alink = new Array(), ignore = new Array();
	var i = 0;
	var msg = discuz$(rlinkmsgid).innerHTML;
	...
	discuz$(rlinkmsgid).innerHTML = msg;
}

function _updatesecqaa(idhash) {
	if(discuz$('secqaa_' + idhash)) {
		discuz$('secqaaverify_' + idhash).value = '';
		if(secST['qaa_' + idhash]) {
			clearTimeout(secST['qaa_' + idhash]);
		}
		discuz$('checksecqaaverify_' + idhash).innerHTML = '<img src="'+ IMGDIR + '/none.gif" width="16" height="16" class="vm" />';
		ajaxget('misc.php?mod=secqaa&action=update&idhash=' + idhash, 'secqaa_' + idhash, null, '', '', function() {
			secST['qaa_' + idhash] = setTimeout(function() {discuz$('secqaa_' + idhash).innerHTML = '<span class="xi2 cur1" onclick="updatesecqaa(\''+idhash+'\')">刷新验证问答</span>';}, 180000);
		});
	}
}
...
function _updateseccode(idhash, play) {
	if(isUndefined(play)) {
		if(discuz$('seccode_' + idhash)) {
			discuz$('seccodeverify_' + idhash).value = '';
			if(secST['code_' + idhash]) {
				clearTimeout(secST['code_' + idhash]);
			}
			discuz$('checkseccodeverify_' + idhash).innerHTML = '<img src="'+ IMGDIR + '/none.gif" width="16" height="16" class="vm" />';
			ajaxget('misc.php?mod=seccode&action=update&idhash=' + idhash, 'seccode_' + idhash, null, '', '', function() {
				secST['code_' + idhash] = setTimeout(function() {discuz$('seccode_' + idhash).innerHTML = '<span class="xi2 cur1" onclick="updateseccode(\''+idhash+'\')">刷新验证码</span>';}, 180000);
			});
		}
	} else {
...
function _checksec(type, idhash, showmsg, recall) {
	...
	var secverify = discuz$('sec' + type + 'verify_' + idhash).value;
	if(!secverify) {
		return;
	}
	var x = new Ajax('XML', 'checksec' + type + 'verify_' + idhash);
	x.loading = '';
	discuz$('checksec' + type + 'verify_' + idhash).innerHTML = '<img src="'+ IMGDIR + '/loading.gif" width="16" height="16" class="vm" />';
	x.get('misc.php?mod=sec' + type + '&action=check&inajax=1&&idhash=' + idhash + '&secverify=' + (BROWSER.ie && document.charset == 'utf-8' ? encodeURIComponent(secverify) : secverify), function(s){
		var obj = discuz$('checksec' + type + 'verify_' + idhash);
		obj.style.display = '';
...
function _setDoodle(fid, oid, url, tid, from) {
	if(tid == null) {
		...
		discuz$(tid).style.display = '';
		discuz$(fid).style.display = 'none';
	}
...
function _showdistrict(container, elems, totallevel, changelevel, containertype) {
	...
	var pid = changelevel >= 1 && elems[0] && discuz$(elems[0]) ? getdid(discuz$(elems[0])) : 0;
	var cid = changelevel >= 2 && elems[1] && discuz$(elems[1]) ? getdid(discuz$(elems[1])) : 0;
	var did = changelevel >= 3 && elems[2] && discuz$(elems[2]) ? getdid(discuz$(elems[2])) : 0;
	var coid = changelevel >= 4 && elems[3] && discuz$(elems[3]) ? getdid(discuz$(elems[3])) : 0;
...
function _showselect(obj, inpid, t, rettype) {
	var showselect_row = function (inpid, s, v, notime, rettype) {
		if(v >= 0) {
			if(!rettype) {
				...
				return '<a href="javascript:;" onclick="discuz$(\'' + inpid + '\').value = \'' + d.getFullYear() + '-' + month + '-' + day + (!notime ? ' ' + hour + ':' + minute: '') + '\'">' + s + '</a>';
			} else {
				return '<a href="javascript:;" onclick="discuz$(\'' + inpid + '\').value = \'' + v + '\'">' + s + '</a>';
			}
		} else if(v == -1) {
			return '<a href="javascript:;" onclick="discuz$(\'' + inpid + '\').focus()">' + s + '</a>';
		} else if(v == -2) {
			return '<a href="javascript:;" onclick="discuz$(\'' + inpid + '\').onclick()">' + s + '</a>';
		}
	};
	...
	if(!obj.id) {
		...
		if(!t) {
			...
			if(discuz$(t)) {
				var lis = discuz$(t).getElementsByTagName('LI');
				for(i = 0;i < lis.length;i++) {
					s += '<a href="javascript:;" onclick="discuz$(\'' + inpid + '\').value = this.innerHTML;discuz$(\''+obj.id+'_menu\').style.display=\'none\'">' + lis[i].innerHTML + '</a>';
				}
				s += showselect_row(inpid, '自定义', -1);
			} else {
				s += '<a href="javascript:;" onclick="discuz$(\'' + inpid + '\').value = \'0\'">永久</a>';
...
function _zoom(obj, zimg, nocover, pn) {
	...
	var showloading = function (zimg, pn) {
		if(!pn) {
			if(!discuz$(menuid + '_waiting')) {
				...
				discuz$('append_parent').appendChild(waiting);
			}
		}
		discuz$(menuid + '_waiting').style.display = '';
		discuz$(menuid + '_waiting').style.left = (document.body.clientWidth - 42) / 2 + 'px';
		discuz$(menuid + '_waiting').style.top = ((document.documentElement.clientHeight - 42) / 2 + Math.max(document.documentElement.scrollTop, document.body.scrollTop)) + 'px';
		loading = new Image();
		setTimeout(function () { loadCheck(loading); }, 100);
		if(!pn) {
			discuz$(menuid + '_zoomlayer').style.display = 'none';
		}
		loading.src = zimg;
	};
	var adjustpn = function(h) {
		h = h < 90 ? 90 : h;
		if(discuz$('zimg_prev')) {
			discuz$('zimg_prev').style.height= parseInt(h) + 'px';
		}
		if(discuz$('zimg_next')) {
			discuz$('zimg_next').style.height= parseInt(h) + 'px';
		}
	};
	var showimage = function (zimg, w, h, imgw, imgh) {
		discuz$(menuid + '_waiting').style.display = 'none';
		discuz$(menuid + '_zoomlayer').style.display = '';
		discuz$(menuid + '_img').style.width = 'auto';
		discuz$(menuid + '_img').style.height = 'auto';
		discuz$(menuid).style.width = (w < 300 ? 300 : w + 20) + 'px';
		mheight = h + 50;
		discuz$(menuid).style.height = mheight + 'px';
		discuz$(menuid + '_zoomlayer').style.height = (mheight < 120 ? 120 : mheight) + 'px';
		discuz$(menuid + '_img').innerHTML = '<img id="' + zoomid + '" src="' + zimg + '" width="' + w + '" height="' + h + '" w="' + imgw + '" h="' + imgh + '" />' + imgtitle;
		if(discuz$(menuid + '_imglink')) {
			discuz$(menuid + '_imglink').href = zimg;
		}
	...
	var adjust = function(e, a) {
		...
		var imgw = discuz$(zoomid).getAttribute('w');
		var imgh = discuz$(zoomid).getAttribute('h');
		...
		if(!a) {
			...
			if(e.wheelDelta <= 0 || e.detail > 0) {
				if(discuz$(zoomid).width - imgwstep <= 200 || discuz$(zoomid).height - imghstep <= 200) {
					doane(e);return;
				}
				discuz$(zoomid).width -= imgwstep;
				discuz$(zoomid).height -= imghstep;
			} else {
				if(discuz$(zoomid).width + imgwstep >= imgw) {
					doane(e);return;
				}
				discuz$(zoomid).width += imgwstep;
				discuz$(zoomid).height += imghstep;
			...
			discuz$(zoomid).width = imgw;
			discuz$(zoomid).height = imgh;
		}
		discuz$(menuid).style.width = (parseInt(discuz$(zoomid).width < 300 ? 300 : parseInt(discuz$(zoomid).width)) + 20) + 'px';
		mheight = (parseInt(discuz$(zoomid).height) + 50);
		discuz$(menuid).style.height = mheight + 'px';
		discuz$(menuid + '_zoomlayer').style.height = (mheight < 120 ? 120 : mheight) + 'px';
		adjustpn(discuz$(zoomid).height);
		setMenuPosition('', menuid, '00');
		doane(e);
	};
	if(!discuz$(menuid) && !pn) {
		...
		if(discuz$(menuid + '_adjust')) {
			discuz$(menuid + '_adjust').onclick = function(e) {adjust(e, 1)};
		}
	}
	showloading(zimg, pn);
	picpage = '';
	discuz$(menuid + '_picpage').innerHTML = '';
	if(typeof zoomgroup == 'object' && zoomgroup[obj.id] && typeof aimgcount == 'object' && aimgcount[zoomgroup[obj.id]]) {
		...
		if(authorlength > 1) {
			...
			if(authorcurrent !== '') {
				...
				picpage += ' <div id="zimg_prev" onmouseover="dragMenuDisabled=true;this.style.backgroundPosition=\'0 50px\'" onmouseout="dragMenuDisabled=false;this.style.backgroundPosition=\'0 -100px\';" onclick="zoom(discuz$(\'aimg_' + paid + '\'), discuz$(\'aimg_' + paid + '\').getAttribute(\'zoomfile\'), 0, 1)" class="zimg_prev"><strong>上一张</strong></div> ';
				paid = authorcurrent < authorlength - 1 ? authorimgs[authorcurrent + 1] : authorimgs[0];
				picpage += ' <div id="zimg_next" onmouseover="dragMenuDisabled=true;this.style.backgroundPosition=\'100% 50px\'" onmouseout="dragMenuDisabled=false;this.style.backgroundPosition=\'100% -100px\';" onclick="zoom(discuz$(\'aimg_' + paid + '\'), discuz$(\'aimg_' + paid + '\').getAttribute(\'zoomfile\'), 0, 1)" class="zimg_next"><strong>下一张</strong></div> ';
			}
			if(picpage) {
				discuz$(menuid + '_picpage').innerHTML = picpage;
			}
		}
	}
...
function _switchTab(prefix, current, total, activeclass) {
	...
	for(var i = 1; i <= total;i++) {
		var classname = ' '+discuz$(prefix + '_' + i).className+' ';
		discuz$(prefix + '_' + i).className = classname.replace(' '+activeclass+' ','').substr(1);
		discuz$(prefix + '_c_' + i).style.display = 'none';
	}
	discuz$(prefix + '_' + current).className = discuz$(prefix + '_' + current).className + ' '+activeclass;
	discuz$(prefix + '_c_' + current).style.display = '';
}

function _initTab(frameId, type) {
	if (typeof document['diyform'] == 'object' || discuz$(frameId).className.indexOf('tab') < 0) return false;
	type = type || 'click';
	var tabs = discuz$(frameId+'_title').childNodes;
	var arrTab = [];
	for(var i in tabs) {
	...
	for(var i = 0;i < len; i++) {
		...
		tab.appendChild(li);
		discuz$(frameId+'_title').removeChild(arrTab[i]);
		counter++;
	}
	discuz$(frameId+'_title').appendChild(tab);
}
...
function switchTabUl (e) {
	...
	for(var j = 0; j < len2; j++) {
		...
		var content = discuz$(tabs[j].id+'_content');
...
function slideshow(el) {
	...
	this.checkLoad = function () {
		...
		var percentEle = discuz$(this.id+'_percent');
...
function _showTip(ctrlobj) {
	...
	if(!discuz$(menuid)) {
		...
		div.className = 'tipwrapper tip_js';
		div.style.display = 'none';
		div.innerHTML = '<div class="tip_horn"></div><div class="tip_c">' + ctrlobj.getAttribute('tip') + '</div>';
		discuz$('append_parent').appendChild(div);
	}
	discuz$(ctrlobj.id).onmouseout = function () { hideMenu('', 'prompt'); };
	showMenu({'mtype':'prompt','ctrlid':ctrlobj.id,'pos':'21','duration':2,'zindex':JSMENU['zIndex']['prompt']});
}
...
function _showPrompt(ctrlid, evt, msg, timeout) {
	...
	if(discuz$(menuid)) {
		discuz$(menuid).parentNode.removeChild(discuz$(menuid));
	}
	var div = document.createElement('div');
	div.id = menuid;
	div.className = ctrlid ? 'tipwrapper tip_js' : 'ntcwin';
	div.style.display = 'none';
	discuz$('append_parent').appendChild(div);
	if (ctrlid) {
	...
	if(ctrlid) {
		...
		if (discuz$(ctrlid)) {
		    if (discuz$(ctrlid).evt !== false) {
		        var prompting = function () {
		            showMenu({ 'mtype': 'prompt', 'ctrlid': ctrlid, 'evt': evt, 'menuid': menuid, 'pos': '210' });
		        };
		        if (evt == 'click') {
		            discuz$(ctrlid).onclick = prompting;
		        } else {
		            //discuz$(ctrlid).onmouseover = prompting;
		        }
		    }
		    showMenu({ 'mtype': 'prompt', 'ctrlid': ctrlid, 'evt': evt, 'menuid': menuid, 'pos': '210', 'duration': duration, 'timeout': timeout, 'zindex': JSMENU['zIndex']['prompt'] });
		    discuz$(ctrlid).unselectable = false;
		}
		else {
		    showMenu({ 'mtype': 'prompt', 'pos': '00', 'menuid': menuid, 'duration': duration, 'timeout': timeout, 'zindex': JSMENU['zIndex']['prompt'],'zindex': 1000000 });
		    discuz$(menuid).style.top = (parseInt(discuz$(menuid).style.top) - 100) + 'px';    
		}
	} else {
...
function creditShow(creditinfo, notice, basev, bk, first, creditrule) {
	...
	for(i = 1; i <= 8; i++) {
		...
		if(discuz$('hcredit_' + i)) {
			discuz$('hcredit_' + i).innerHTML = basev[i];
		}
	}
	for(i = 1; i <= 8; i++) {
		if(notice[i] != 0) {
...
function _showColorBox(ctrlid, layer, k, bgcolor) {
	...
	if(!discuz$(ctrlid + '_menu')) {
		...
		for(var i = 0; i < 40; i++) {
			...
			+ (typeof wysiwyg == 'undefined' ? 'seditor_insertunit(\'' + k + '\', \'[' + tag1 + '=' + coloroptions[i] + ']\', \'[/' + tag1 + ']\')' : (ctrlid == editorid + '_tbl_param_4' ? 'discuz$(\'' + ctrlid + '\').value=\'' + coloroptions[i] + '\';hideMenu(2)' : 'discuzcode(\'' + tag2 + '\', \'' + coloroptions[i] + '\')'))
...
function _extstyle(css) {
	if(!discuz$('css_extstyle')) {
		loadcss('extstyle');
	}
	discuz$('css_extstyle').href = css ? css + '/style.css' : STATICURL + 'image/common/extstyle_none.css';
	currentextstyle = css;
	setcookie('extstyle', css, 86400 * 30);
	if(discuz$('css_widthauto') && !discuz$('css_widthauto').disabled) {
...
function _widthauto(obj) {
	if(discuz$('css_widthauto')) {
		CSSLOADED['widthauto'] = 1;
	}
	if(!CSSLOADED['widthauto'] || discuz$('css_widthauto').disabled) {
		if(!CSSLOADED['widthauto']) {
			loadcss('widthauto');
		} else {
			discuz$('css_widthauto').disabled = false;
		}
		HTMLNODE.className += ' widthauto';
		setcookie('widthauto', 1, 86400 * 30);
		obj.innerHTML = '切换到窄版';
	} else {
		discuz$('css_widthauto').disabled = true;
...
function _imageRotate(imgid, direct) {
	...
	if(BROWSER.ie) {
		...
		var canvas = discuz$(image.getAttribute('canvasid'));
		if(!canvas) {
...
function _createPalette(colorid, id, func) {
	...
	if (!discuz$("c"+colorid+"_menu")) {
		var dom = document.createElement('span');
		dom.id = "c"+colorid+"_menu";
		dom.style.display = 'none';
		dom.innerHTML = iframe;
		discuz$('append_parent').appendChild(dom);
	}
