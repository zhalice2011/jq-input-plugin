...
{   
     ...
     for (j = 0; j < scriptCount; j++) {
         scriptFileName = script[j];
         if (!JavascrptLoaded[scriptFileName]) {
             appendscript(scriptFileName);             
         }
     }
 }

 function checkAllLoaded(script) {
     var ret = true;
     var scriptCount = script.length;
     for (j = 0; j < scriptCount; j++) {
         scriptFileName = script[j];
...
function appendscript(src, text, reload, charset)
{
    ...
    var agentKey = 'firefox';
    var isFirefox = USERAGENT.indexOf(agentKey) != -1;
    scriptNode.charset = charset ? charset : (isFirefox ? document.characterSet : document.charset);
    try
    {
        if (src)
        {
