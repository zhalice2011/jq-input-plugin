...
$.Autocompleter = function(input, options) {
	...
	function selectCurrent() {
		...
		options.onSelected(selected.data) ;
		return true;
	}
	...
	function request(term, success, failure) {
		...
		// if an AJAX url has been supplied, try loading the data now
			...
			$.ajax({
				...
				url: options.url + '?house='+encodeURIComponent(term),
				data: $.extend({
					q: lastWord(term),
					limit: options.max
				}, extraParams),
		...
		for (var i=0; i < rows.length; i++) {
			...
			if (row) {
				parsed[parsed.length] = {
					data: json[i],
					value:json[i].name,
					result: options.formatResult && options.formatResult(json[i].name, json[i].name) || json[i].name
				};
			}
		}
		return parsed;
